...
export default function MovieDetailModal({ movie, isOpen, onClose, onMovieSelect }: MovieDetailModalProps) {
  ...
  const uploadImageMutation = useMutation({
    mutationFn: async (file: File) => {
      console.log('[IMAGE] Starting upload for movie:', movie?.id, movie?.title);
      ...
      console.log('[IMAGE] Upload successful, received URL:', result.imageUrl);
      return result;
    },
    onMutate: () => {
      console.log('[IMAGE] Setting upload state to true');
      setIsUploadingPoster(true);
    },
    onSuccess: async (data) => {
      console.log('[IMAGE] Updating movie with image URL:', data.imageUrl);
      try {
        // Update the movie with the new image URL
        const updateResponse = await apiRequest(`/api/movies/${movie.id}`, {
          method: 'PATCH',
          body: JSON.stringify({ posterUrl: data.imageUrl }),
        });
        
        console.log('[IMAGE] Movie update response:', updateResponse);
        console.log('[IMAGE] Movie updated, invalidating cache');
        ...
        toast({
          title: "Movie image updated successfully",
          description: "Your custom image has been uploaded and saved.",
        });
      } catch (error) {
        console.error('[IMAGE] Error updating movie:', error);
        setIsUploadingPoster(false);
        toast({
                ...
                <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-60 transition-all duration-200 flex items-center justify-center">
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                    ...
                    <label
                      ...
                      className="cursor-pointer bg-orange-600 hover:bg-orange-700 text-white px-3 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2"
                    >
                      <Upload size={16} />
                      {isUploadingPoster ? 'Uploading...' : 'Upload New Image'}
                    </label>
                  </div>
                </div>
              </div>
                          ...
                          <div className="text-sm text-gray-400 mt-1">
                            ...
                            {version.condition && (
                              <span className="ml-2 text-yellow-400">
                                {"★".repeat(Math.max(0, Math.min(5, version.condition || 0)))}{"☆".repeat(Math.max(0, 5 - Math.min(5, version.condition || 0)))}
                              </span>
                            )}
                            {version.resaleValue && (
